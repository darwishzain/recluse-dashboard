...
def start():
    ...
    meetBtn = tk.Button(tab0, text="Open GMeet",command=lambda:openLink("https://meet.google.com"))
    meetBtn.grid(row=0, column=1)

    window.mainloop()

def openLink():
    os.system('cmd /c "start explorer https://meet.google.com"')

def openLink(url):
    link = 'cmd /c \"start explorer '+url+'\"'
    print(link)
